...
var checkPalindromeFormation = function(a, b) {
  ...
  while(i<j){
    if(a[i]!==b[j]){
      if(checkStrFromIndex(a,a,i,j) || checkStrFromIndex(b,b,i,j)){
        return true
      }else{
        break
      }
...
function checkStrFromIndex(a,b,i,j){
  while(i<j){
    if(a[i]!==b[j]){
      return false
    }
...
console.log(checkPalindromeFormation('pvhmupgqeltozftlmfjjde','yjgpzbezspnnpszebzmhvp'))
